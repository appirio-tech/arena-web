...
/**
 * Create a new instance of C# Processor
 ...
 * @constructor
 */
function CSharpProcessor(className, methodName, originalCode) {
    ...
    function addClass(name, start, end, isComparator) {
        ...
        if (isComparator === "IComparer" || isComparator === "IComparable") { //check if class is a Comparator class
            newClass[4] = "true";
        } else {
            newClass[4] = "false";
        }
        ...
        for (i = 0; i < code.length; i++) {
            if (code[i] === '{') {
                ...
                if (ok && currentLocation[currentLocation.length - 1] !== "inMethod") {
                    isClass = currentLocation[currentLocation.length - 1] === "nothing" || code.lastIndexOf("class ", i) > code.lastIndexOf("(", i) || code.lastIndexOf("struct ", i) > code.lastIndexOf("(", i);
    ...
    function countAutoCodeInOriginalCode() {
        var auto = ["// Powered by FileEdit", "// Powered by CodeProcessor", "// Powered by PopsEdit",
                "// Powered by [KawigiEdit]", "//Powered by [KawigiEdit]", "// Powered by TCGen for C#"],
        ...
        for (i = 0; i < auto.length; i++) {
            ...
            if (autoIndex > rightBrace) {
                for (k = 0; k < auto[i].length; k++) {
                    if (auto[i][k] !== ' ' && auto[i][k] !== '\t' && auto[i][k] !== '\n' && auto[i][k] !== '\r') {
                        count++;
                    }
                }
            }
    ...
    function countImports() {
        ...
        while (true) {
            importStart = code.indexOf("using", cur);
            ...
            cur = nextEnter + 1;
            for (k = importStart; k <= nextEnter; k++) {
...
module.exports = CSharpProcessor;